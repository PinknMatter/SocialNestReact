    ...
    fetchData()
      .then((graph) => {
        ...
        const simulation = d3
          ...
          .force("charge", d3.forceManyBody().strength(-80)) // Try making the charge more negative
      ...
      .catch((error) => {
        ...
        // Previously, you might have also considered calling setIsLoading(false) in case of error
      })
      .finally(() => {
        // This block will execute after the then or catch block has executed

        setIsLoading(false); // Hide the loading indicator regardless of the fetch outcome
      });
  }, []);
